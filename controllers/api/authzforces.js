...
var debug = require('debug')('idm:api-authzforce_controller');
...
exports.submit_authzforce_policies = function(application_id, submit_authzforce) {
	...
	return new Promise(function(resolve, reject) {
		var domain = { oauth_client_id: application_id }
		...
		models.authzforce.findOne({
			...
			var create_policy = { message: '', status: authzforce_response[1].status }
			var activate_policy = { message: '', status: authzforce_response[2].status }
			...
			resolve({create_policy: create_policy, activate_policy: activate_policy});
