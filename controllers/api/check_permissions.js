    ...
    if (!req.application) {
      ...
      owned_permissions(req.token_owner.id, req.application.id)
        .then(function(user_policies) {
          ...
          return obtain_defined_policies(
            req.originalUrl,
            req.method,
            req.params
          );
        ...
        .then(function(needed_permissions) {
          req.needed_permissions = needed_permissions;
          if (
            check_permissions(req.user_owned_permissions, needed_permissions)
          ) {
            resolve(true);
          } else {
            reject({
...
function obtain_defined_policies(url, method, parameters) {
  return new Promise(function(resolve) {
    ...
    const needed_permissions = application_policy.actions[key]
      ? application_policy.actions[key].split(',')
      : [];
