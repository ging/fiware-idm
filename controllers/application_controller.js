	...
	application.validate().then(function(err) {
		application.save({fields: ['id', 'name', 'description', 'url', 'redirect_uri', 'secret', 'image']}).then(function() {
			models.role.bulkCreate( 
				[ 	
					{name: 'Provider',  oauth_client_id: application.id},
					{name: 'Purchaser', oauth_client_id: application.id}
				]
            ).then(function(){
            	models.role.findOne({ where: { name: 'Provider', oauth_client_id: application.id } }).then(function(role) {
