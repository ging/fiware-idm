...
exports.role_permissions_assign = function(req, res) {
  ...
  const roles_id = Object.keys(JSON.parse(req.body.submit_assignment));

  // Filter req.body and obtain an array without roles provider and purchaser
  const public_roles_id = roles_id.filter(
  ...
  if (public_roles_id.length > 0) {
    models.role_permission
      ...
      .then(function() {
        const submit_assignment = JSON.parse(req.body.submit_assignment);
        ...
        for (const role in submit_assignment) {
          if (!['provider', 'purchaser'].includes(role)) {
            for (
              let permission = 0;
              permission < submit_assignment[role].length;
              permission++
            ) {
        ...
        models.role_permission
          .bulkCreate(create_assign_roles_permissions)
          .then(function() {
            if (config_authorization.authzforce.enabled) {
              authzforce_controller.submit_authzforce_policies(
                req,
                res,
                submit_assignment
              );
