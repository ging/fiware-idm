...
function logAuthState() {
  ...
  if (
      config.session.secret ===  "nodejs_idm" || 
      config.password_encryption == "nodejs_idm'"

  ) {
    /* eslint-disable no-console */
    console.log(stars);
    console.log(
     "WARNING: Encryption keys must be changed if you are using the IDM in a production Environment");
    console.log(
      '        These keys should be set using Docker Secrets'
    );
    console.log(stars);
    /* eslint-enable no-console */
  }
  ...
  if (
    ...
    console.log(stars);
    console.log(
     "WARNING: It is recommended that you reconfigure the IDM database access not to use default values"
    console.log(
      '        These keys should be set using Docker Secrets'
    );
    console.log(stars);
...
function processEnvironmentVariables() {
  ...
  if (process.env.IDM_ENCRYPTION_KEY) {
    config.password_encryption = process.env.IDM_ENCRYPTION_KEY;
  }
  ...
  if (process.env.IDM_CORS_ENABLED) {
    ...
    if (process.env.IDM_CORS_METHODS) {
      config.cors.methods = to_array(process.env.IDM_CORS_METHODS, [
        'GET',
        'HEAD',
        'PUT',
        'PATCH',
  ...
  if (process.env.IDM_OAUTH_EMPTY_STATE) {
    config.oauth2.allow_empty_state = to_boolean(
      process.env.IDM_OAUTH_EMPTY_STATE,
      false
    );
  }
  if (process.env.IDM_OAUTH_AUTH_LIFETIME) {
    config.oauth2.authorization_code_lifetime =
      process.env.IDM_OAUTH_AUTH_LIFETIME;
  }
  ...
  if (process.env.IDM_OAUTH_UNIQUE_URL) {
    config.oauth2.unique_url = to_boolean(
      process.env.IDM_OAUTH_UNIQUE_URL,
      false
    );
  }
  ...
  if (process.env.IDM_AUTHZFORCE_ENABLED) {
    config.authorization.authzforce = to_boolean(
      process.env.IDM_AUTHZFORCE_ENABLED,
      false
    );
  ...
  if (process.env.IDM_USAGE_CONTROL_ENABLED) {
    config.usage_control.enabled = to_boolean(
      process.env.IDM_USAGE_CONTROL_ENABLED,
      false
    );
  ...
  if (process.env.IDM_EX_AUTH_ENABLED) {
    config.external_auth.enabled = to_boolean(
      process.env.IDM_EX_AUTH_ENABLED,
      false
    );
    ...
    if (process.env.IDM_EX_AUTH_PASSWORD_ENCRYPTION) {
      config.external_auth.password_encryption =
        process.env.IDM_EX_AUTH_PASSWORD_ENCRYPTION;
    }
    if (process.env.IDM_EX_AUTH_ID_PREFIX) {
      config.external_auth.id_prefix = process.env.IDM_EX_AUTH_ID_PREFIX;
    }
    ...
    if (process.env.IDM_EX_AUTH_PASSWORD_ENCRYPTION_KEY) {
      config.external_auth.password_encryption_key =
        process.env.IDM_EX_AUTH_PASSWORD_ENCRYPTION_KEY;
    }
    ...
    if (process.env.IDM_EX_AUTH_DB_USER_TABLE) {
      config.external_auth.database.user_table =
        process.env.IDM_EX_AUTH_DB_USER_TABLE;
    }
  ...
  if (process.env.IDM_EIDAS_ENABLED) {
    ...
    if (process.env.IDM_EIDAS_GATEWAY_HOST) {
      config.eidas.metadata_expiration =
        process.env.IDM_EIDAS_METADATA_LIFETIME;
    }
  }
}
