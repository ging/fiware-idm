...
function process_environment_variables(verbose) {
  ...
  if (process.env.IDM_REDIRECT) {
    config.registration = config.registration || {};
    config.registration.redirect = process.env.IDM_REDIRECT;
    if (process.env.IDM_EXTENSION) {
      config.registration.extension = process.env.IDM_EXTENSION;
    }
  }
