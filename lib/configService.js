...
function process_environment_variables(verbose) {
  ...
  if (process.env.IDM_CSP_SCRIPT_SRC) {
    const script_src = process.env.IDM_CSP_SCRIPT_SRC.split(',');
  ...
  // Config VC external Authentication
  config.external_vc = config.external_vc || {};
  if (process.env.IDM_EXTERNAL_VC_ENABLED) {
    config.external_vc.enabled = to_boolean(process.env.IDM_EXTERNAL_VC_ENABLED, false);

    config.external_vc.jwks.credential_location = process.env.IDM_CREDENTIAL_LOCATION;

    if (process.env.IDM_JWKS_HOST) {
      config.external_vc.jwks.host = process.env.IDM_JWKS_HOST;
    }
    if (process.env.IDM_JWKS_PATH) {
      config.external_vc.jwks.path = process.env.IDM_JWKS_PATH;
    }
    if (process.env.IDM_JWKS_KID) {
      config.external_vc.jwks.kid = process.env.IDM_JWKS_KID;
    }
  }
