...
const debug = require('debug')('idm:database');
...
function make_request(callback) {
  ...
  sequelize
    ...
    .catch((err) => {
      if(err.parent.fatal){
        debug(`Warning: Connection refused - ${database.host}:${database.port || 3306} - ${err.parent.code} `);
        callback(1);
      } else if(err.parent.code === 'ER_BAD_DB_ERROR'){
        debug(`Warning: Connected, but DB not initialized  - ${err.parent.sqlMessage}`);
        callback(0);
      } else {
        debug(`Error: Unknown error - ${err.parent.code}`);
        callback(1);
      }
    });
}
  ...
  const connect_with_retry = () => {
    make_request((err) => {
      if (err) {
        ...
        if (retry === 0) {
          debug('Fatal: Unable to connect after multiple attempts. Exiting');
          process.exit(1);
        } else {
          debug('Info: Retrying after 5 seconds.');
