...
const logging = require('debug')('idm:sass');
...
module.exports = function (options) {
  ...
  const force = options.force || options.response;
  // Enable debug output
  const debug = options.debug;
  // Enable beep on error
  const beep = options.beepOnError || false;

  const sassExtension = options.indentedSyntax === true ? '.sass' : '.scss';
  ...
  const log =
    options.log ||
    function (severity, key, val, text) {
      ...
      if (severity === 'error') {
        logging('[sass]  \x1B[90m%s:\x1B[0m \x1B[36m%s %s\x1B[0m', key, val, text);
      } else {
        logging('[sass]  \x1B[90m%s:\x1B[0m \x1B[36m%s %s\x1B[0m', key, val, text);
      }
    };
  ...
  return function sass(req, res, next) {
    ...
    const error = function (err, errorMessage) {
    ...
    if (root) {
      ...
      sassPath = join(root, src, path.replace(new RegExp('^' + dest), '').replace(/\.css$/, sassExtension));
    ...
    const done = function (err, result) {
      if (err) {
        ...
        const fileLineColumn = file + ':' + err.line + ':' + err.column;
        const errorMessage =
          (beep ? '\x07' : '') + '\x1B[31m' + err.message.replace(/^ +/, '') + '\n\nin ' + fileLineColumn + '\x1B[91m';
      ...
      mkdirp(dirname(cssPath), '0700').then(() => {
      ...
      if (sourceMap) {
        const sourceMapPath = this.options.sourceMap;
    ...
    const compile = function () {
      fs.exists(sassPath, function (exists) {
        ...
        const style = options.compile();

        const renderOptions = util._extend({}, options);
