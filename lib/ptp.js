...
exports.submit_policies = function(
  application_id,
  previous_job_id,
  submit_ptp
) {
  ...
  return new Promise(function(resolve, reject) {
    const body = JSON.stringify(
      parse_body_request(application_id, previous_job_id, submit_ptp)
    );
...
function parse_body_request(application_id, previous_job_id, submit_ptp) {
  const submit_policies = JSON.parse(
    JSON.stringify(require('../templates/ptp/policies.json'))
  );
  submit_policies.id = application_id;
  submit_policies.previousJobId = previous_job_id;
  for (let i = 0; i < submit_ptp.length; i++) {
    const rule = JSON.parse(
      JSON.stringify(require('../templates/ptp/rule.json'))
    );
      ...
      for (param in submit_ptp[i].parameters) {
        ...
        if (param === 'aggregate_time') {
          rule.rule.params[param_camel_case] =
            submit_ptp[i].parameters[param].value * 1000;
        } else {
          rule.rule.params[param_camel_case] =
            submit_ptp[i].parameters[param].value;
        }

        if (submit_ptp[i].parameters[param].unit) {
          rule.rule.params[param_camel_case] =
            rule.rule.params[param_camel_case] *
            times_seconds[submit_ptp[i].parameters[param].unit];
        }
      }
    }
    rule.punishment = submit_ptp[i].punishment;
