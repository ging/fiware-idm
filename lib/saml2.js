...
module.exports.ServiceProvider = ServiceProvider = (function() {
  ...
  ServiceProvider.prototype.create_authn_request_xml = function(identity_provider, options) {
    ...
    var context = options.auth_context
    var signed = signed.replace('</saml2p:AuthnRequest>', '').trim()

    var add_extension = {
      "saml2p:AuthnRequest": {
        'saml2p:Extensions' : options.extensions,
        'saml2p:NameIDPolicy': {
          '@AllowCreate': 'true',
          '@Format': options.nameid_format || 'urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified'
        },
        'saml2p:RequestedAuthnContext': {
          '@Comparison': context.comparison,
          'saml2:AuthnContextClassRef': context.AuthnContextClassRef[0]
        }
      }
    }
    var add_extension_xml = xmlbuilder.create(add_extension).end()

    var add_extension_xml = add_extension_xml.replace('<?xml version="1.0"?>','').replace('<saml2p:AuthnRequest>', '').trim()

    return Buffer.from(signed + add_extension_xml).toString('base64')
  };
