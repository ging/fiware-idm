$(document).ready(function(){
    ...
    $("#auth_admins").find('.form-control').bind("keyup input",function(e) {
    	$('#spinner_admins').show('open')
    ...
    function send_filter_request() {
    	$.get(url+'?key='+$("#auth_admins").find('.form-control').val().toUpperCase(), function(data, status) {
			...
			if (data.admin_users.length > 0) {
				...
				$('#admins_content').find('.alert').show()
			}
		})
    }
	...
	function admins_pagination(max) {
		...
		$('#auth_admins_pagination_container').bootpag({
