class EventManager {
  constructor() {
    ...
    this.input_filter_user = $('input.form-control#filter_user', this.users_page);
    ...
    this.edit_input_description = $('textarea#id_description', this.form_edit_user);
    this.edit_input_website = $('input#id_website', this.form_edit_user);
    this.edit_input_extra = $('textarea#id_extra', this.form_edit_user);

    // Change password modal
    this.modal_change_password = $('div#change_password_modal');
    this.form_change_password = $('form#change_password_form', this.modal_change_password);
    this.chg_pw_input_password1 = $('input#id_password1', this.form_change_password);
    this.chg_pw_input_password2 = $('input#id_password2', this.form_change_password);
  ...
  init_ui() {
    ...
    this.users_page.on('click', 'input.form-check-input:not(#check_all)', this.change_check_indiv);

    // Select user action event: change password or edit
    this.users_page.on('changed.bs.select', '.selectpicker:not(#select_entries)', this.select_user_action);

    // Change password events
    this.form_change_password.submit(this.submit_password);
    this.modal_change_password.on('hidden.bs.modal', this.close_change_password);
  ...
  select_user_action(e, clickedIndex, newValue, oldValue) {
    var selected = $(e.target).find('option').eq(clickedIndex).val();
    var user_id = $(e.target).closest('tr').attr('id');
  ...
  open_change_password(user) {
    this.form_change_password.attr('action', '/idm/admins/list_users/users/' + user.id + '/change_password');
  ...
  submit_password(event) {
    ...
    if (!$('span.alert', this.form_change_password).is(':visible')) {
      ...
      var put_request = $.ajax({
        url: url,
        type: 'PUT',
        beforeSend: before_send(this.form_change_password.find('input:hidden[name=_csrf]').val()),
        data: data
      });
      ...
      put_request.fail((response) => {
        ...
        for (var i in errors) {
          $('span.alert#' + errors[i].message, this.form_change_password).show('open');
        }
      });
    }
  }
  ...
  open_edit_user(user) {
    this.form_edit_user.attr('action', '/idm/admins/list_users/users/' + user.id + '/edit_info');
    ...
    this.edit_input_extra.val(JSON.stringify(user.extra));
  ...
  submit_edit_user(event) {
    ...
    if (!$('span.alert', this.form_edit_user).is(':visible')) {
      ...
      var put_request = $.ajax({
        url: url,
        type: 'PUT',
        beforeSend: before_send(this.form_edit_user.find('input:hidden[name=_csrf]').val()),
        data: data
      });
      ...
      put_request.done((response) => {
        ...
        users[user_index].extra = response.user.extra;
      ...
      put_request.fail((response) => {
        ...
        var errors = JSON.parse(response.responseText).errors;
        for (var i in errors) {
          $('span.alert#' + errors[i].message, this.form_edit_user).show('open');
        }
      });
    }
  }
  ...
  submit_delete_user(event) {
    ...
    var delete_request = $.ajax({
      url: url,
      type: 'DELETE',
      beforeSend: before_send(this.form_delete_user.find('input:hidden[name=_csrf]').val()),
      data: data
    });
    ...
    if (users_selected.length <= 0) {
      ...
      for (var i = 0; i < users_selected.length; i++) {
        this.list_users_to_delete.append(
          '<li><a href="/idm/users/' + users_selected[i] + '">' + users_selected[i] + '</a></li>'
        );
      }
    }
  }
  ...
  submit_enable_user(event) {
    ...
    var user_id = $(event.target).closest('tr').attr('id');
    var user_current_enablement = $(event.target).siblings("input[type='checkbox']").is(':checked');
    ...
    var put_request = $.ajax({
      ...
      type: 'PUT',
      beforeSend: before_send(this.users_page.find('input:hidden[name=_csrf]').val()),
      data: data
    });
  ...
  submit_create_user(event) {
    ...
    var data = {
      ...
      send_email: $('input#id_send_email', this.form_create_user).is(':checked'),
    ...
    if (!$('span.alert').is(':visible')) {
      ...
      var post_request = $.post({
        url: url,
        data: data,
        beforeSend: before_send(this.form_create_user.find('input:hidden[name=_csrf]').val())
      });

      post_request.done((user) => {
        users.push(user);

        if (this.input_filter_user.val() !== '' && user.username.includes(this.input_filter_user.val())) {
      ...
      post_request.fail((response) => {
        ...
        for (var i in errors) {
          $('span.alert#' + errors[i].message, this.form_create_user).show('open');
        }
      });
    }
  }
