class EventManager {
    ...
    submit_password(event) {
        ...
        if (!$('span.alert', this.form_change_password).is(":visible")) {
            ...
            put_request.done((response) => {
                ...
                this.ui.render_message({message: 'User password change', type: 'success'})
            });

            put_request.fail((response) => {
                this.ui.render_message({message: 'Change user password fail', type: 'danger'})
    ...
    submit_enable_user(event) {
        ...
        put_request.done(() => {
            $('#'+user_id).find('td.enable').find("input[type='checkbox']").prop('checked', !user_current_enablement)
