class EventManager {
	...
	constructor() {
		...
		this.filter_user = $('input.form-control#filter_user', this.users_page);
	...
	init_ui() {
		...
		this.users_page.find('.form-control').bind("keyup input", this.filter_users)
	}
	...
	change_check_all(event) {
		...
		$('tr', this.user_table).each((index) => {
	        ...
	        if (checked) {
	          	if (index < 0) {
	            	$(event.target).find('input.form-check-input').prop('checked', true);
	            	users_selected.push(user_id)
	          	}
	        } else {
	          	if (index > -1) {
	            	$(event.target).find('input.form-check-input').prop('checked', false);
	            	users_selected.splice(index, 1);
	          	}   
	        }
	    });
    ...
    submit_password(event) {
        ...
        if (!$('span.alert', this.form_change_password).is(":visible")) {
            ...
            var put_request = $.ajax({
                url: url,
                type: 'PUT',
                beforeSend: beforeSend(this.form_change_password.find('input:hidden[name=_csrf]').val()),
                data: data
            })
    ...
    submit_edit_user(event) {
        ...
        if (!$('span.alert', this.form_edit_user).is(":visible")) {
            ...
            var put_request = $.ajax({
                url: url,
                type: 'PUT',
                beforeSend: beforeSend(this.form_edit_user.find('input:hidden[name=_csrf]').val()),
                data: data
            })
    ...
    submit_delete_user(event) {
        ...
        var delete_request = $.ajax({
            url: url,
            type: 'DELETE',
            beforeSend: beforeSend(this.form_delete_user.find('input:hidden[name=_csrf]').val()),
            data: data
        })
    ...
    open_delete_user(event) {
    	...
    	if (users_selected.length <= 0) {
    		...
    		for (var i = 0; i < users_selected.length; i++) {
    			this.list_users_to_delete.append('<li><a href="/idm/users/'+users_selected[i]+'">'+users_selected[i]+'</a></li>')
    		}
    	}
    }
    ...
    submit_enable_user(event) {
        ...
        var put_request = $.ajax({
            ...
            type: 'PUT',
            beforeSend: beforeSend(this.users_page.find('input:hidden[name=_csrf]').val()),
            data: data
        })
    ...
    submit_create_user(event) {
        ...
        if (!$('span.alert').is(":visible")) {
            ...
            var post_request = $.post({
                url: url, 
                data: data,
                beforeSend: beforeSend(this.form_create_user.find('input:hidden[name=_csrf]').val()) 
            });

            post_request.done((user) => {
                users.push(user)

                if (this.filter_user.val() !== '' && user.username.includes(this.filter_user.val())) {
    ...
    filter_users(event) {
    	var filter = this.filter_user.val();
        users_filter = []
