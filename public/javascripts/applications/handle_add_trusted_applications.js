...
$(document).ready(function() {
    ...
    $("#add_trusted_app").find('#filter_available_applications').bind("keyup input",function(e) {
        var input = $(this).val();
        clearTimeout(typingTimerMembers);
        typingTimerMembers = setTimeout(function() { available_applications(input, 'available_application_row_template', 'available_applications')}, doneTypingInterval);
    });
    ...
    $("#filter_trusted_applications").bind("keyup input",function(e) {
        ...
        if($("#add_trusted_app").find("#trusted_applications_list").children(':visible').length == 0) {
            $("#no_trusted_applications").show() 
        } else {
    ...
    $("#available_applications").on("click",".active", function(event) { 
        ...
        if ($("#add_trusted_app").find('#trusted_applications_list').find('#'+application_id).length) {
            ...
            $("span#info_added_app").fadeIn(800).delay(300).fadeOut(800);
        } else {

            var trusted_app_row = $('#trusted_application_row_template').html();
            trusted_app_row = trusted_app_row.replace(/application_name/g, htmlEntities(application_name));
            trusted_app_row = trusted_app_row.replace(/application_id/g, String(application_id));
            trusted_app_row = trusted_app_row.replace(/application_image/g, String(application_image));
            $("#add_trusted_app").find("#trusted_applications_list").append(trusted_app_row);

            var info_added_app = "<span id='info_added_app' style='display: none; text-align: center;' class='help-block alert alert-success'>Application "+application_name+" added</span>"
            $("span#info_added_app").replaceWith(info_added_app);
            $("span#info_added_app").fadeIn(800).delay(300).fadeOut(800);
        }
    });
    ...
    $(".trusted-applications").on("click",".remove", function(event) { 
        ...
        var info_added_app = "<span id='info_added_app' style='display: none; text-align: center;' class='help-block alert alert-success'>Application "+application_name+" removed from application</span>"
        $("span#info_added_app").replaceWith(info_added_app);
        $("span#info_added_app").fadeIn(800).delay(300).fadeOut(800);
    ...
    $("#add_trusted_app").find('.cancel, .close').click(function () {
