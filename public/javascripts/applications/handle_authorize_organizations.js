...
$(document).ready(function(){
    	...
    	$.get(url, function(data) {
    		...
    		if (data.type === 'error') {
				...
				for (var i = 0; i < organizations_authorized.length; i++) {
					if (!organization_role_count[organizations_authorized[i].organization_id]) {
						organization_role_count[organizations_authorized[i].organization_id] = 0;
					}
					if (organizations_authorized[i].role_id) {
						organization_role_count[organizations_authorized[i].organization_id]++;						
					}
				}
                ...
                if(organizations_authorized.length > 0) {
    				for (var i = 0; i < organizations_authorized.length; i++) {
    					...
    					if (!$("#authorize_organization").find("#authorized_organizations").find('#'+organization_row).length) {
    				        ...
    				        if (organization_role_count[organizations_authorized[i].organization_id] > 0) {
    				        	assign_role_user_row = assign_role_user_row.replace(/roles_count/g, String(organization_role_count[organizations_authorized[i].organization_id] + " roles"));
    				        } else {
    ...
    $("#authorize_organization").find('#available_update_organizations').bind("keyup input",function(e) {
        ...
        typingTimerMembers = setTimeout(function() {available_organizations(input, 'table_row_available_organization_template', 'available_organizations')}, doneTypingInterval);
    });
    ...
    $("#available_organizations").on("click",".active", function(event) { 
        ...
        var organization_image = row.parent().find(".avatar").children('img').first().attr('src')

        if ($("#authorize_organization").find('ul.update_owners_users_members').find('#'+organization_id).length) {
	        	info_added_organization = "<span id='info_added_organization' style='display: none; text-align: center;' class='help-block alert alert-warning'>Organization "+organization_name+" has been already added</span>"
	        	$("#authorize_organization").find("#info_added_organization").replaceWith(info_added_organization);
	        	$("#authorize_organization").find("#info_added_organization").fadeIn(800).delay(300).fadeOut(800);
	    } else {
    ...
    $("#authorized_organizations").on("click",".remove", function(event) { 
        ...
        var info_added_organization = "<span id='info_added_organization' style='display: none; text-align: center;' class='help-block alert alert-success'>Organization "+organization_name+" removed from application</span>"
    	$("#authorize_organization").find("#info_added_organization").replaceWith(info_added_organization);
    	$("#authorize_organization").find("#info_added_organization").fadeIn(800).delay(300).fadeOut(800);
    ...
    $("#authorized_organizations").on("click",".role_dropdown_role", function(event) { 

    	// Stop linking        
        event.stopPropagation();

        var role_id = String($(this).attr("id"))
        var role_organization = String($(this).closest('.app-roles-list').attr('id'))
