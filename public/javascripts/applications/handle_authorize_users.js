...
$(document).ready(function(){
    ...
    var application_name = $('#detailApplication').find('h4.name').contents().get(0).nodeValue.trim()
    	...
    	$.get(url, function(data) {
    		...
    		if (data.type === 'error') {
                ...
                if (users_authorized.length > 0) {
    				for (var i = 0; i < users_authorized.length; i++) {
    					if (!$("#authorize_user").find("#authorized_users").find("#"+users_authorized[i].user_id).length) {
    				        ...
    				        assign_role_user_row = assign_role_user_row.replace(/application_name/g, htmlEntities(application_name));
    				        ...
    				        $("#authorize_user").find("#authorized_users").append(assign_role_user_row);
    				        for (j in roles) {
    				        	var role = "<li id="+roles[j].id+" class='role_dropdown_role'><i class='fa fa-check'></i>"+roles[j].name+"</li>"
    				        	$("#authorize_user").find("#authorized_users").find("#"+users_authorized[i].user_id).find("ol").append(role)
    				        }
    					}
    					if (users_authorized[i].role_id) {
    						$("#authorize_user").find("#authorized_users").find("#"+users_authorized[i].user_id).find("#"+users_authorized[i].role_id).addClass("active")
    					}
    				}    			
                } else {
    ...
    $("#available_members").on("click",".active", function(event) { 
        ...
        if ($("#authorize_user").find('ul.update_owners_users_members').find('#'+user_id).length) {
	        ...
	        for (j in roles) {
	        	var role = "<li id="+roles[j].id+" class='role_dropdown_role'><i class='fa fa-check'></i>"+roles[j].name+"</li>"
	        	$("#authorize_user").find("#authorized_users").find("#"+user_id).find("ol").append(role)
	        }
		...
		if (event.which == 13) {
			...
			for (var key in user_role_count) {
				if (user_role_count[key] == 0) {	
					...
					$("#authorize_user").find("#authorized_users").find("#"+key).find(".role_options").addClass("dropdown-empty")
				}
			}

			if ($("#authorize_user").find("#authorized_users").find(".dropdown-empty").length === 0 || $("#authorize_user").find(".modal-footer").find("#submit_button").val() == "Confirm") {
...
function exit_authorize_users() {
    ...
    $("#authorize_user").find(".alert-warning").hide();
    $("#authorize_user").find("#no_update_owners_users_members").hide()
    ...
    $("#authorize_user").find("#authorized_users").empty();
