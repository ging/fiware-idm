...
$(document).ready(function() {
  ...
  $('#update_owners_permissions').on('click', '.list-group-item', function(e) {
    if (
      $(e.target).hasClass('fa-edit') === false &&
      $(e.target).hasClass('fa-trash-alt') === false
    ) {
  ...
  $('#permButton').click(function() {
    var action =
      '/idm/applications/' + application.id + '/edit/permissions/create';
      ...
      success: function(result) {
        ...
        if (result.type === 'warning') {
          ...
          for (var i = result.text.length - 1; i >= 0; i--) {
            $('#create_permission_form')
              .find(
                '#' + result.text[i].message + '.help-block.alert.alert-danger'
              )
              .show('open');
          }
          return false;
          $('#create_permission_form')
            ...
            .hide('close');
          $('#create_permission_form')
            .find('#error_invalid_inputs.help-block.alert.alert-danger')
            .show('open');
          ...
          if (method === 'POST') {
            ...
            permission = permission.replace(
              'perm_name',
              htmlEntities(result.permission.name)
            );
            permission = permission.replace(
              'perm_id',
              String(result.permission.id)
            );
  ...
  $('#update_owners_permissions').on('click', '.edit_permission', function(
    event
  ) {
    // stop form from submitting normally
    event.preventDefault();
      ...
      success: function(result) {
        if (result.type === 'success') {
          ...
          for (var role in application.role_permission_assign) {
            application.role_permission_assign[
              role
            ] = application.role_permission_assign[role].filter(function(
              permission
            ) {
          ...
          application.permissions = application.permissions.filter(function(
            elem
          ) {
