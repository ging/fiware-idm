...
$(document).ready(function(){
    ...
    var application_id = $('#assign_role_permission_form').attr('action').split('/')[3]
    ...
    function parse_values(roles, permissions) {
        ...
        for (var i = 0; i < roles.length; i++) {
            ...
            if (['provider', 'purchaser'].includes(roles[i].id)) {                                         
                ...
                $('#update_owners_roles').find('#'+roles[i].id).children('a').remove()
            } else {
        ...
        for (var i = 0; i < permissions.length; i++) {
            ...
            permission_row = permission_row.replace(/perm_name/g, htmlEntities(permissions[i].name));
            if (['1', '2', '3' ,'4' ,'5' ,'6'].includes(permissions[i].id)) {                            
                $('#list_permissions').prepend(permission_row);
                $('#list_permissions').find('[data-permission-id='+permissions[i].id+']').children('span').remove();
                $('#list_permissions').find('[data-permission-id='+permissions[i].id+']').children('.edit_permission').remove();
                $('#list_permissions').find('[data-permission-id='+permissions[i].id+']').children('.delete').remove();
            } else {
