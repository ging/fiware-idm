...
(function(factory) {
  ...
  $.fn.serializeJSON = function(options) {
    ...
    $.each(formAsArray, function(i, obj) {
      ...
      if (type !== 'skip') {
        // ignore inputs with type 'skip'
        keys = f.splitInputNameIntoKeysArray(nameWithNoType);
        parsedValue = f.parseValue(value, name, type, opts); // convert to string, number, boolean, null or customType

        skipFalsy =
          !parsedValue &&
          f.shouldSkipFalsy($form, name, nameWithNoType, type, opts); // ignore falsy inputs if specified
        if (!skipFalsy) {
  ...
  $.serializeJSON = {
    ...
    parseValue: function(valStr, inputName, type, opts) {
      ...
      if (opts.typeFunctions && type && opts.typeFunctions[type]) {
        ...
        // make sure to apply :string type if it was re-defined
