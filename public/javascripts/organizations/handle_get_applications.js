$(document).ready(function(){
	...
	function load_applications(url, panel) {
		...
		$.get(url, function(data, status) {
			if (data.applications.length > 0) {
				...
				create_applications_row(data.applications, $('#applications_content'))	
			} else {
				$('#applications_content').find('.alert').show('open')
			}
    ...
    $("#auth_applications").find('.form-control').bind("keyup input",function(e) {
    	$('#spinner_applications').show('open')
    	clearTimeout(typingTimerApplications);
        typingTimerApplications = setTimeout(send_filter_request, doneTypingInterval);
    });

    function send_filter_request() {
    	$.get(url+'?key='+$("#auth_applications").find('.form-control').val().toUpperCase(), function(data, status) {
			$('#applications_content').children('.list-group-item').remove()
			$('#spinner_applications').hide('close')
			if (data.applications.length > 0) {
				$('#applications_content').find('.alert').hide()
				applications_pagination(data.applications_number)
				create_applications_row(data.applications, $('#applications_content'));
			} else {
				$('#applications_pagination_container').empty()
				$('#applications_content').find('.alert').show()
			}
		})
    }
	...
	function applications_pagination(max) {
		...
		$('#applications_pagination_container').bootpag({
