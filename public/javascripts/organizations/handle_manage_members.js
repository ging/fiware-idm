$(document).ready(function(){    
    ...
    var members = []
    var org_name = $('#detailOrganization').find('h3.name').contents().get(0).nodeValue.trim()
        ...
        $.get(url, function(data) {
            if (data.error) {
                ...
                for (var i = 0; i < data.members.length; i++) {
                    ...
                    if (members.some(elem => elem.user_id === data.members[i].user_id) === false) {
                        var org_role_user_row = $('#table_row_org_role_user_template').html();
                        org_role_user_row = org_role_user_row.replace(/username/g, htmlEntities(data.members[i].username));
                        org_role_user_row = org_role_user_row.replace(/user_id/g, String(data.members[i].user_id));
                        org_role_user_row = org_role_user_row.replace(/user_avatar/g, String(data.members[i].image));
                        org_role_user_row = org_role_user_row.replace(/role_id/g, String(data.members[i].role));
                        org_role_user_row = org_role_user_row.replace(/organization_name/g, htmlEntities(org_name));
                        if (data.members[i].role === 'owner') {
                            members_column.prepend(org_role_user_row)
                        } else {
                            members_column.append(org_role_user_row)                            
                        }
                        members_column.find('#'+user).find('#'+role).addClass('active')
                        members.push({ user_id: data.members[i].user_id, role: data.members[i].role})
                    } 
                }                
            }
        });
    ...
    $("#authorize_user").find(".members").on("click",".role_dropdown_role", function(event) { 
        ...
        if ($(this).hasClass("active")) {
            ...
            $(this).closest('.role_options').addClass("not_assigned")
        ...
        // Add role to user
            ...
            $(this).closest('.role_options').removeClass("not_assigned")
    ...
    $("#available_members").on("click",".active", function(event) { 
        ...
        if ($("#authorize_user").find('ul.update_owners_users_members').find('#'+user_id).length) {
            ...
            members_column.find('#'+user_id).find('#member').addClass('active')
    ...
    $("#submit_managed_members_form").bind("keypress submit", function(event) {
        ...
        if (event.which == 13) {
            ...
            if (not_assigned.length === 0 || $("#authorize_user").find("#submit_button").val() == "Confirm") {
                // get the action attribute from the <form action=""> element 
                var $form = $(this),
                    url = $form.attr('action');

                // Change value of hidden input
                $('#authorize_user').find('#submit_members').val(JSON.stringify(members))

                // Continue with the submit request
                $('#submit_managed_members_form')[0].submit();
            } else {
                $("#authorize_user").find("#submit_button").val("Confirm")
            }
        }
    });
