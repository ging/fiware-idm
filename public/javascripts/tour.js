...
var _toursOptions = {
	...
	appsTourOptions: {
		...
		},
		{
			...
			title: "STEP 1: Registering a new application",
			content: "These are the OAuth2 Grant Types that your application will allow.",
			placement: "left",
			onShown: function (tour) {
				var def = $.Deferred();
				setTimeout(function(){
					$('.dropdown-menu').addClass("tour-step-backdrop-parent").css("z-index", "1101");
					$("label[for='id_grant_type']").siblings('div').find('.bootstrap-select').addClass('open')
	                def.resolve();
				})
		...
		},
		{
			...
			title: "STEP 1: Registering a new application",
			content: "This is to select who is going be the provider of the application: yourself or one of the organizations in which you are owner.",
			placement: "left",
			onShown: function (tour) {
				var def = $.Deferred();
				setTimeout(function(){
					$('.dropdown-menu').addClass("tour-step-backdrop-parent").css("z-index", "1101");
					$("label[for='id_provider']").siblings('div').find('.bootstrap-select').addClass('open')
	                def.resolve();
				})
	...
	orgsTourOptions: {
		...
		},
		{
			...
			element: "#content_body #auth_users",
			title: "Members",
			content: "These are the members of the organization.",
			placement: "left"
		},
		{
			path: RegExp("\/idm\/organizations\/[^\/]", "i"),
			...
			title: "Applications",
			content: "These are the applications in which the organization is authorized.",
			placement: "left"
		},
		{
			path: "/idm",
			title: "You're all set!",
			content: "<p>You finished the Organizations Tour! You can now head on to Help&About to learn more about KeyRock or start experimenting yourself.</p><p>Thank you for using FIWARE Lab!</p><a href='/idm/help_about' class='next-tour' data-current-tour='orgsTour' id='end-tours'> > Go to Help&About</a>",
			orphan: true,
			redirect: function () {
