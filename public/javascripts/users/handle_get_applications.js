$(document).ready(function(){
	...
	function load_applications(url, panel) {
		...
		$.get(url, function(data, status) {
			if (data.applications.length > 0) {
				...
				create_applications_row(data.applications, $('#auth_applications_content'))	
			} else {
				$('#auth_applications_content').find('.alert').show('open')
			}
    ...
    function send_filter_request() {
    	$.get(url+'?key='+$("#auth_applications").find('.form-control').val().toUpperCase(), function(data, status) {
			$('#auth_applications_content').children('.list-group-item').remove()
			$('#spinner_applications').hide('close')
			if (data.applications.length > 0) {
				$('#auth_applications_content').find('.alert').hide()
				applications_pagination(data.applications_number)
				create_applications_row(data.applications, $('#auth_applications_content'));
			} else {
				$('#auth_applications_pagination_container').empty()
				$('#auth_applications_content').find('.alert').show()
			}
		})
    }
	...
	function applications_pagination(max) {
		...
		$('#auth_applications_pagination_container').bootpag({
