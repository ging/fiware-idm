...
const resource_requests = utils.readExampleFile(
  './test/templates/api/012-resource_request.json'
);
...
describe('API - 12 - fiware service permission request: ', function() {
  ...
  before(function(done) {
    const get_oauth_token = {
      ...
      headers: {
        Authorization:
          'Basic ' +
          Buffer.from(application_id + ':' + application_secret).toString(
            'base64'
          ),
  ...
  describe('2) When requesting a resource with resource, action and fiware_service that is permitted', function() {
    it('should return a 200 OK and a Permit', function(done) {
      const list_permissions = {
        url:
          config.host +
          '/user?access_token=' +
          oauth_access_token +
          '&action=POST&resource=/login&fiware_service=test&app_id=' +
          application_id,
        method: 'GET',
      };
      request(list_permissions, function(error, response, body) {
  ...
  describe('3) When requesting a resource with resource, action and wrong fiware_service that is not permitted', function() {
    it('should return a 200 OK and a Deny', function(done) {
      const list_permissions = {
        url:
          ...
          '/user?access_token=' +
          oauth_access_token +
          '&action=GET&resource=login&fiware_service=wrong&app_id=' +
          application_id,
        method: 'GET',
      };
      request(list_permissions, function(error, response, body) {
  ...
  describe('4) When requesting a resource with fiware_service, action and wrong resource that is not permitted', function() {
    it('should return a 200 OK and a Deny', function(done) {
      const list_permissions = {
        url:
          config.host +
          '/user?access_token=' +
          oauth_access_token +
          '&action=GET&resource=wrong&fiware_service=test&app_id=' +
          application_id,
        method: 'GET',
      };
      request(list_permissions, function(error, response, body) {
