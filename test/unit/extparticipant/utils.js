...
describe('External Participant Controller Utils: ', () => {
  ...
  describe('validate_client_certificate', () => {
    ...
    describe('should not validate certificates with errors', () => {
      ...
      test('invalid signature algorithm', { signatureOid: 'a' });
      test('no serial number', { serialNumber: '' });
      test('Key usage is for CA', {
        getExtension: () => {
          return {
      ...
      test('Key usage is for signing CRLs', {
        getExtension: () => {
          return {
            digitalSignature: false,
            keyCertSign: false,
  ...
  describe('validate_participant_from_jwt', () => {
    ...
    it('should manage errors querying satellite server for an access token', async () => {
      ...
      utils.verifier = {
        verify: sinon.mock().returns(
          Promise.resolve({
            payload:
              '{"parties_info": {"count": 1, "data": [{"adherence": {"status": "Active"}, "party_name": "Party Name"}]}}'
          })
        )
      };
    ...
    it('should return party name if the client is a valid participant', async () => {
      ...
      const party_name = await utils.validate_participant_from_jwt(client_payload, CERTIFICATE_WITH_SPECIAL_CHARACTERS);
